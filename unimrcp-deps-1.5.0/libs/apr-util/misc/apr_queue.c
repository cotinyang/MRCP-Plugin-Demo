...
/**
 * Push new data onto the queue. Blocks if the queue is full until the 
 * the timeout is reached. Once the push operation has completed, it 
 * signals other threads waiting in apr_queue_pop() that they may 
 * continue consuming sockets.
 */
APU_DECLARE(apr_status_t) apr_queue_timedpush(apr_queue_t *queue,
 ...
 * the timeout is reached. Once retrieved, the item is placed 
 * into the address specified by 'data'.
 */
APU_DECLARE(apr_status_t) apr_queue_timedpop(apr_queue_t *queue,
                                             apr_interval_time_t timeout,
                                             void **data)
{
    apr_status_t rv;
