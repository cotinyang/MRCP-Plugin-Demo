...
typedef struct aneex_recog_state_machine_t aneex_recog_state_machine_t;
struct aneex_recog_state_machine_t {
	/** state machine base */
...
static APR_INLINE apt_bool_t aneex_recog_response_dispatch(aneex_recog_state_machine_t *state_machine, mrcp_message_t *message)
{
...
static APR_INLINE apt_bool_t aneex_recog_event_dispatch(aneex_recog_state_machine_t *state_machine, mrcp_message_t *message)
{
	if(state_machine->base.active == FALSE) {
...
static APR_INLINE void aneex_recog_state_change(aneex_recog_state_machine_t *state_machine, aneex_recog_state_e state, mrcp_message_t *message)
{
...
static apt_bool_t recog_request_get_params(aneex_recog_state_machine_t *state_machine, mrcp_message_t *message)
{
	return aneex_recog_request_dispatch(state_machine,message);
}
...
static apt_bool_t aneex_recog_request_recognize(aneex_recog_state_machine_t *state_machine, mrcp_message_t *message)
{
	...
	if(state_machine->state == RECOGNIZER_STATE_RECOGNIZING) {
		...
		return aneex_recog_response_dispatch(state_machine,response);
	}
...
static apt_bool_t aneex_recog_pending_requests_remove(aneex_recog_state_machine_t *state_machine, mrcp_message_t *request_message, mrcp_message_t *response_message)
{
...
static apt_bool_t aneex_recog_request_state_update(aneex_recog_state_machine_t *state_machine, mrcp_message_t *message)
{
	...
	apt_log(APT_LOG_MARK,APT_PRIO_INFO,"Process %s Request " APT_SIDRES_FMT " [%" MRCP_REQUEST_ID_FMT "]",
		...
		MRCP_MESSAGE_SIDRES(message),
		message->start_line.request_id);
	method = aneex_recog_request_method_array[message->start_line.method_id];
	if(method) {
...
static apt_bool_t aneex_recog_response_state_update(aneex_recog_state_machine_t *state_machine, mrcp_message_t *message)
{
	recog_method_f method;
	...
	apt_log(APT_LOG_MARK,APT_PRIO_INFO,"Process %s Response " APT_SIDRES_FMT " [%" MRCP_REQUEST_ID_FMT "]",
		...
		MRCP_MESSAGE_SIDRES(message),
		message->start_line.request_id);
	method = aneex_recog_response_method_array[message->start_line.method_id];
	if(method) {
		return method(state_machine,message);
	}
	return aneex_recog_response_dispatch(state_machine,message);
...
static apt_bool_t aneex_recog_event_state_update(aneex_recog_state_machine_t *state_machine, mrcp_message_t *message)
{
	...
	apt_log(APT_LOG_MARK,APT_PRIO_INFO,"Process %s Event " APT_SIDRES_FMT " [%" MRCP_REQUEST_ID_FMT "]",
		...
		MRCP_MESSAGE_SIDRES(message),
		message->start_line.request_id);
	method = aneex_recog_event_method_array[message->start_line.method_id];
	if(method) {
		return method(state_machine,message);
	}
	return aneex_recog_event_dispatch(state_machine,message);
...
static apt_bool_t aneex_recog_state_update(mrcp_state_machine_t *base, mrcp_message_t *message)
{
	...
	switch(message->start_line.message_type) {
		case MRCP_MESSAGE_TYPE_REQUEST:
			status = aneex_recog_request_state_update(state_machine,message);
			break;
		case MRCP_MESSAGE_TYPE_RESPONSE:
			status = aneex_recog_response_state_update(state_machine,message);
			break;
		case MRCP_MESSAGE_TYPE_EVENT:
			status = aneex_recog_event_state_update(state_machine,message);
			break;
		default:
			status = FALSE;
			break;
...
static apt_bool_t aneex_recog_state_deactivate(mrcp_state_machine_t *base)
{
	aneex_recog_state_machine_t *state_machine = (aneex_recog_state_machine_t*)base;
...
mrcp_state_machine_t* aneex_recog_state_machine_create(void *obj, mrcp_version_e version, apr_pool_t *pool)
{
	...
	state_machine->properties = mrcp_message_header_create(
			...
			aneex_recog_header_vtable_get(version),
			pool);
